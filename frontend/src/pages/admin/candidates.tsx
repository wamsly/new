...
export default function AdminCandidatesPage() {
  ...
  const pollDetails = useGetPollDetails(pollId, {
    query: {
  ...
  const refetch = () =>
    qc.invalidateQueries({ queryKey: getAdminListCandidatesQueryKey() });

  const filtered = useMemo(
    () =>
      (candidates as any[]).filter((c) => {
        ...
        if (search) {
          ...
          return (
            ...
            c.pollTitle?.toLowerCase().includes(n) ||
  ...
  const handleApprove = async (id: string) => {
    try {
      await approve.mutateAsync({ candidateId: id });
  ...
  const handleReject = async () => {
    if (!rejectFor) return;
    try {
      await reject.mutateAsync({ candidateId: rejectFor, data: { reason } });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Approve, reject or add candidates manually.
          </p>
        </div>
        <Dialog open={openAdd} onOpenChange={setOpenAdd}>
          ...
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Manually add a candidate to a seat. They will be marked as
                approved.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleAdd} className="space-y-3">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    {(pollDetails.data?.seats ?? []).map((s: any) => (
              ...
              <div>
                ...
                <Select value={userId} onValueChange={setUserId}>
                  <SelectTrigger>
                    <SelectValue placeholder="Pick a registered student" />
                  </SelectTrigger>
                  <SelectContent>
                    {((users.data ?? []) as any[])
                      .filter(
        ...
        <CardHeader>
          <div className="flex flex-wrap items-center gap-3">
            <Input
              ...
              placeholder="Search candidate, poll or seat…"
            ...
            <CardTitle className="text-sm font-medium text-muted-foreground">
              {filtered.length} of {(candidates as any[]).length}
            </CardTitle>
          </div>
        </CardHeader>
        <CardContent>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Provide a reason. The candidate will see this on their dashboard.
            </DialogDescription>
          </DialogHeader>
          <Textarea
            rows={4}
      ...
      <Dialog
        open={Boolean(viewDocs)}
        onOpenChange={(o) => !o && setViewDocs(null)}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Documents — {viewDocs?.name}</DialogTitle>
            <DialogDescription>
              Files uploaded by the candidate during their application.
            </DialogDescription>
          </DialogHeader>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground capitalize">
                    {d.documentType} ·{" "}
                    {new Date(d.uploadedAt).toLocaleDateString("en-KE")}
                  </p>
                </div>
                <ExternalLink className="h-3.5 w-3.5 text-muted-foreground" />
              </a>
            ))}
            {(viewDocs?.documents ?? []).length === 0 && (
