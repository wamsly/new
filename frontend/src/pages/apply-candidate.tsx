...
export default function ApplyCandidatePage() {
  ...
  const { data: pollDetails } = useGetPollDetails(pollId, {
    query: {
      ...
      queryKey: getGetPollDetailsQueryKey(pollId),
    },
  });
  ...
  const seats = useMemo(() => {
    const allSeats = pollDetails?.seats ?? [];
    return allSeats.filter((s: any) => {
      if (!s.eligible) return false;
      // Allow re-application if the existing application for this seat is rejected
      const existingApp = (applications as any[]).find(
        (a) => a.seatId === s.id,
      );
      return !existingApp || existingApp.status === "rejected";
    });
  }, [pollDetails, applications]);
  ...
  const addFile = useCallback((file: File, type: "document" | "photo") => {
    ...
    reader.onload = (e) => {
      setUploads((prev) => [
        ...prev,
        { name: file.name, type, file, dataUrl: e.target?.result as string },
      ]);
    };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!canApply) {
      toast.error("The application window is closed or not yet open.");
      return;
    }
    try {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Submit your manifesto and supporting documents to stand as a
          candidate.
        </p>
      </div>
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              You can only apply for seats you are eligible to vote on, and only
          ...
          <CardContent>
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              {pollId && (
                <div
                  className={`rounded-lg border px-4 py-3 text-sm flex items-start gap-3 ${canApply ? "border-green-500/40 bg-green-500/10" : "border-destructive/40 bg-destructive/10"}`}
                >
                  {canApply ? (
                    <Clock className="mt-0.5 h-4 w-4 text-green-600 shrink-0" />
                  ) : (
                  ...
                  <div>
                    {!settings ? (
                      ...
                      <div>
                        ...
                        <p className="text-muted-foreground">
                          Please wait for the admin to open the application
                          window.
                        </p>
                      </div>
                    )}
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  disabled={!pollId || !canApply}
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        pollId ? "Choose a seat" : "Pick a poll first"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Brief personal background and experience"
                  disabled={!canApply}
                />
              </div>
              ...
              <div className="space-y-2">
                <Label>
                  Manifesto <span className="text-destructive">*</span>
                </Label>
                <Textarea
                  rows={6}
                  required
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    e.target.files?.[0] &&
                    addFile(e.target.files[0], "document")
                  }
                />
                <input
                  ref={photoInputRef}
                  ...
                  accept="image/*"
                  onChange={(e) =>
                    e.target.files?.[0] && addFile(e.target.files[0], "photo")
                  }
                />
                {uploads.length > 0 && (
              ...
              <Button
                type="submit"
                className="w-full"
                disabled={apply.isPending || uploadDoc.isPending || !canApply}
              >
                {apply.isPending || uploadDoc.isPending ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Status of submissions you've made.
            </CardDescription>
          </CardHeader>
